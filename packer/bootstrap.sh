...
cat <<EOF | sudo tee /etc/buildkite-agent/buildkite-agent.cfg
token="$BUILDKITE_TOKEN"
name="$BUILDKITE_AGENT_NAME"
tags="queue=$BUILDKITE_QUEUE,gcp-project=$PROJECT_ID,gcp-zone=$ZONE,gcp-instance=$INSTANCE_NAME,$BUILDKITE_TAGS"
priority=$BUILDKITE_PRIORITY
...
shell-options="-euo pipefail"
timestamp-lines=true
health-check-addr=":8080"
debug=false
EOF
...
cat <<EOF | sudo tee /etc/buildkite-agent/env
# Environment variables for Buildkite agent
BUILDKITE_AGENT_TOKEN=$BUILDKITE_TOKEN
BUILDKITE_AGENT_NAME=$BUILDKITE_AGENT_NAME
BUILDKITE_GCP_PROJECT_ID=$PROJECT_ID
BUILDKITE_GCP_ZONE=$ZONE
BUILDKITE_GCP_INSTANCE_NAME=$INSTANCE_NAME
EOF
...
echo ""
echo "You can:"
echo "  • Check status: sudo systemctl status buildkite-agent"
echo "  • View logs: sudo journalctl -u buildkite-agent -f"
echo "  • Check config: sudo cat /etc/buildkite-agent/buildkite-agent.cfg"